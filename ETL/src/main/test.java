...
//			test.execute(args);
			...
			System.out.println("***** " + edit_date + " 日次バッチ開始 *****");

			test.executeTRUNCATE();

			test.executeSelect();
			edit_date = (df.format(cal.getTime()));
			System.out.println("***** " + edit_date + " 日次バッチ終了 *****");
		}catch(Throwable t) {
			t.printStackTrace();
		}finally{
			// オブジェクトを破棄
...
//		}
	...
	private void executeSelect()
		throws SQLException{
		edit_date = (df.format(cal.getTime()));
		System.out.println("***** " + edit_date + " 顧客データ取り込み開始 *****");
...
//				System.out.println("***** " + edit_date + " データSELECT *****");
			...
			edit_date = (df.format(cal.getTime()));
			System.out.println("***** " + edit_date + " 顧客データ取り込み終了 取り込み件数=" + RecordCount + "件 *****");
		}finally{
